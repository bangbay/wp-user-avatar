...
class WP_User_Avatar {
  function __constuct(){
    ...
    register_activation_hook(WPUA_DIR.'wp-user-avatar.php', 'wpua_options');
    // Remove subscribers edit_posts capability
    register_deactivation_hook(WPUA_DIR.'wp-user-avatar.php', 'wpua_deactivate');
    // Translations
    add_action('plugins_loaded', array($this, 'wpua_load_translations'));
  }

  function wp_user_avatar(){
    global $current_user, $pagenow, $wpua_allow_upload;
    // Add WPUA to profile
    if(current_user_can('upload_files') || ((bool) $wpua_allow_upload == 1 && is_user_logged_in())){
      // Profile functions and scripts
      add_action('show_user_profile', array($this, 'wpua_action_show_user_profile'));
      ...
      // Admin scripts
      if($pagenow == 'options-discussion.php' || ($pagenow == 'admin.php' && isset($_GET['page']) && $_GET['page'] == 'wp-user-avatar-settings')){
  ...
  // Load translations
  function wpua_load_translations(){
